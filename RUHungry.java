...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        categoryVar = new String[StdIn.readInt()];
        menuVar = new MenuNode[categoryVar.length];
        for (int i = 0; i < categoryVar.length; i++)
        {
            ...
            MenuNode last = null;
            int k = 0; 
            while (k < numberofDishes)
            {
                String dishRead = StdIn.readString() + StdIn.readLine(); 
                int[] stockID = new int[StdIn.readInt()];
                for (int m = 0; m < stockID.length; m++)
                {
                    stockID[m] = StdIn.readInt();
                }
                Dish dish = new Dish(categoryVar[i], dishRead, stockID);
